...
class ClientManager:
    """Manages per-user SplitwiseClient instances with caching.

    Each user (identified by their Splitwise access token) gets their own
    SplitwiseClient so requests are isolated.
    """

    def __init__(self, consumer_key: str, consumer_secret: str, cache_ttl: int = 86400):
        self.consumer_key = consumer_key
        self.consumer_secret = consumer_secret
        self.cache_ttl = cache_ttl
        self._clients: Dict[str, SplitwiseClient] = {}

    def get_client(self, access_token: Optional[str] = None) -> SplitwiseClient:
        """Return a SplitwiseClient for the given access token.

        If *access_token* is ``None`` the value is read from the
        per-request ``current_access_token`` context variable.
        """
        token = access_token or get_access_token()
        if token is None:
            raise RuntimeError("No Splitwise access token in current context")
        ...
        if token not in self._clients:
            ...
            self._clients[token] = SplitwiseClient(auth, cache_ttl=self.cache_ttl)
            logger.info("Created new SplitwiseClient for user")
